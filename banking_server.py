...
from utils import broadcast, object_to_txt


class BankingServer:


    def transcation(self, lamport_clock: LamportClock, queue: PriorityQueue, balance_table: BalanceTable, block_chain: BlockChain, receiver: str, amount: float, replies, clients):
        
        # put yourself in the queue
        queue.insert(lamport_clock)

        # send request to all
        broadcast("REQUEST" + "|" + object_to_txt(lamport_clock), clients)
        ...
        broadcast("BLOCK" + "|" + object_to_txt(block), clients)

        # send release to all
        broadcast("RELEASE", clients)
